...
def generate_mermaid_code(input_text):
    ...
    try:
        logger.debug("Starting generate_mermaid_code with input: %s", input_text[:100])  # Print first 100 chars of input
        ...
        logger.debug("Received response from Gemini API: %s", response.text[:100] if response.text else "No response")
        
        if response.text:
            cleaned_code = clean_mermaid_code(response.text)
            logger.debug("Cleaned mermaid code: %s", cleaned_code[:100])
            return cleaned_code
        else:
            logger.error("No response text received from Gemini API")
            raise Exception("پاسخی از مدل دریافت نشد")
            
    except Exception as e:
        logger.error("Detailed error in generate_mermaid_code: %s", str(e))
        logger.error("Error type: %s", type(e))
        import traceback
        logger.error("Full traceback:")
        logger.error(traceback.format_exc())
        raise Exception(f"خطا در تولید کد Mermaid: {str(e)}")
...
def generate():
    try:
        ...
        text = data['text']
        logger.debug(f"Received text for processing: {text[:100]}")  # Print first 100 chars
        
        mermaid_code = generate_mermaid_code(text)
        if not mermaid_code:
            logger.error("No mermaid code generated")
            return jsonify({'error': 'Failed to generate Mermaid code'}), 500
            
        logger.debug(f"Generated mermaid code: {mermaid_code[:100]}")  # Print first 100 chars
...
def extract_text():
    try:
        ...
        # ایجاد پرامپت برای استخراج متن
        prompt = """
        لطفاً متن موجود در این تصویر را به طور کامل و دقیق استخراج کن.
        نکات مهم:
        1. تمام متن را به فارسی برگردان
        2. ساختار و فرمت متن را حفظ کن
        3. هیچ توضیح اضافه یا متن تکمیلی اضافه نکن
        فقط متن استخراج شده را برگردان.
        """
        ...
        if response.text:
            ...
            return jsonify({'error': 'متنی از تصویر استخراج نشد'}), 400
